...
export default function VerifyPage() {
  ...
  const { register, handleSubmit, formState, setError } = useForm<Answers>({ defaultValues: {} });

  async function onSubmit(values: Answers) {
    try {
        ...
        <form className="mt-6 space-y-5" onSubmit={handleSubmit(onSubmit)}>
          {prompts.map((p: any) => (
            <Input
              ...
              label={`${p.label} *`}
              required
              error={(formState.errors as any)?.[p.key]?.message}
              {...register(p.key, {
                required: "This answer is required",
                minLength: { value: 1, message: "This answer is required" },
